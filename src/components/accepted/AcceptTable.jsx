...
const getApplicationData = async (token, query) => {
    ...
    try{
      ...
      if(currPage == "accepted"){
          const result = await authAxios.post(`/api/admin/search`, query)
          .then((value) =>{
            rows = [];
              ...
              for (var i = 0; i < value.data.applications.length; i++){
                rows.push(
                  createData(
                    value.data.applications[i].first_name + " " + value.data.applications[i].last_name,
                    value.data.applications[i].age,
                    value.data.applications[i].email, 
                    value.data.applications[i].aadharNumber,
                  ),
                )
              }
  ...
  function handleQuerySubmit(){
    const token = window.localStorage.getItem("token").toString(); 
    getApplicationData(token, props.query);
  }

  async function downloadFullList(){
    const token = window.localStorage.getItem("token").toString(); 
  ...
  const result = await authAxios.post(`/api/admin/download`)
  .then((value) =>{
